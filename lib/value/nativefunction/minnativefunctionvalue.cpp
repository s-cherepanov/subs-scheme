    ...
    for( ; it != argvalues->end(); ++it )
    {
        ...
        else
        {
            ...
            if( !intop )
            {
                ...
                throw EvaluationError( "Invalid argument for min: '"
    ...
    const CombinationValue* argvalues ) const
{
    ...
    if( it == argvalues->end() )
    {
        ArgsChecker::ThrowNotEnoughArgsException( "min", argvalues->size(), 1 );
    }
    ...
    // We start with the first operand as the minimum
...
//virtual
MinNativeFunctionValue* MinNativeFunctionValue::Clone() const
{
    return new MinNativeFunctionValue( *this );
}


//virtual
std::string MinNativeFunctionValue::GetName() const
{
    return StaticName();
}

//static
const std::string& MinNativeFunctionValue::StaticName()
{
    static const string static_name( "min" );
    return static_name;
}
